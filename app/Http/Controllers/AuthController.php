...
class AuthController extends Controller
{
    ...
    public function ClientRegister (Request $request) {
        ...
        $validation_custom_phone = DB::table('khachhang')->where('kh_sdt','=',$sdt)->first();
        ...
        else if ($validation_custom_email)
        {
            $success = Session::put('alert-error', 'Email đã được đăng ký');
            return redirect()->route('dangkykhachhang');
        }
        else if ($validation_custom_phone)
        {
            $success = Session::put('alert-error', 'Số điện thoại đã được đăng ký');
            return redirect()->route('dangkykhachhang');
        }
...
// cap nhat thong tin khach hang
    ...
    public function updatepassword(Request $request, $username)
    {
        ...
        {
            $success = Session::put('alert-info2', 'Vui lòng nhập mật khẩu mới !!!');
        }
        else if($request->matkhau == $request->matkhau2){
